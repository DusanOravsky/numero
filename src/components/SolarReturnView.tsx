  ...
  return (
    <GlassCard>
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        <label className="text-sm text-slate-400">Rok:</label>
        <input
          type="number"
          min={birthYear}
          max={birthYear + 120}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
        <div className="p-3 rounded-xl bg-amber-500/10 border border-amber-500/30">
          <p className="text-xs text-amber-300 uppercase mb-1">Dátum návratu</p>
          <p className="text-sm text-white font-medium">
            {sr.date.toLocaleDateString('sk-SK', { day: 'numeric', month: 'long', year: 'numeric' })}
          </p>
          <p className="text-[11px] text-slate-400">
            Vek pri návrate: {sr.ageAtReturn} r. · čas: {sr.date.toLocaleTimeString('sk-SK', { hour: '2-digit', minute: '2-digit' })}
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {sun && (
          <div className="p-3 rounded-xl bg-yellow-500/10 border border-yellow-500/30">
            <p className="text-xs text-yellow-300 uppercase mb-1">Slnko SR</p>
            <p className="text-sm text-white">{sun.symbol} {sun.sign.symbol} {sun.sign.name} {sun.degree.toFixed(1)}°</p>
            <p className="text-[11px] text-slate-400">
              {sr.result.planetHouses.Slnko ? `${sr.result.planetHouses.Slnko}. dom` : ''} — kde žiari vaša identita tento rok.
            </p>
          </div>
        )}
        {moon && (
          <div className="p-3 rounded-xl bg-indigo-500/10 border border-indigo-500/30">
            <p className="text-xs text-indigo-300 uppercase mb-1">Mesiac SR</p>
            <p className="text-sm text-white">{moon.symbol} {moon.sign.symbol} {moon.sign.name} {moon.degree.toFixed(1)}°</p>
            <p className="text-[11px] text-slate-400">
              {sr.result.planetHouses.Mesiac ? `${sr.result.planetHouses.Mesiac}. dom` : ''} — kde sú vaše emocionálne potreby.
            </p>
          </div>
        )}
      </div>
      ...
      <p className="text-[11px] text-slate-500 italic mt-3">
        Solar Return má najsilnejšiu výpovednú hodnotu pre rok od narodenín do narodenín. Niektoré školy
        odporúčajú tráviť narodeniny na špecifickom mieste — tým možno "preladiť" SR ascendent.
      </p>
    </GlassCard>
  );
}
