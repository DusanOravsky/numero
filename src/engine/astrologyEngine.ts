...
export function calculateSolarReturn(
  ...
  let localHr = returnHr + returnTz;
  let adjustedD = returnD;
  let adjustedM = returnM;
  let adjustedY = returnY;
  if (localHr >= 24) {
    localHr -= 24;
    const nextDay = new Date(Date.UTC(adjustedY, adjustedM - 1, adjustedD + 1));
    adjustedD = nextDay.getUTCDate();
    adjustedM = nextDay.getUTCMonth() + 1;
    adjustedY = nextDay.getUTCFullYear();
  } else if (localHr < 0) {
    localHr += 24;
    const prevDay = new Date(Date.UTC(adjustedY, adjustedM - 1, adjustedD - 1));
    adjustedD = prevDay.getUTCDate();
    adjustedM = prevDay.getUTCMonth() + 1;
    adjustedY = prevDay.getUTCFullYear();
  }
  return {
    date: returnDate,
    result: calculateAstrology(adjustedD, adjustedM, adjustedY, localHr, returnMin, latitude, longitude, timezoneOffsetHours),
